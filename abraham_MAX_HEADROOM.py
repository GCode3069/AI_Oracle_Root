...
USE_CASHAPP = True  # Use Cash App QR (more user-friendly for mobile)
...
def generate_script(headline, style='cursor_consistent', ctr_level='moderate'):
    """
    Generate DARK SATIRICAL COMEDY with MULTI-STYLE support
    
    Styles (from Battle Royale competitive intelligence):
    - cursor_consistent: Safe, formulaic (8-12% CTR)
    - chatgpt_poetic: Memorable, sophisticated (12-18% CTR)
    - grok_controversial: Edgy, trend-jacking (10-25% CTR, use caution)
    - opus_sophisticated: Multi-layered, nuanced (10-15% CTR)
    
    CTR Levels:
    - safe: 8-10% target
    - moderate: 10-15% target
    - aggressive: 15-20% target
    - maximum: 20-25% target (high risk)
    
    NO Bitcoin address in audio - QR code only!
    """
    ...
    try:
        ...
        if style == 'chatgpt_poetic':
            ...
            risk = 'moderate' if ctr_level in ['safe', 'moderate'] else 'maximum'
            return generator.grok_style(headline, risk_level=risk)
        elif style == 'opus_sophisticated':
            return generator.opus_style(headline)
        else:
            # Fall through to cursor_consistent (original formula)
            pass
    except:
        # Fallback to original if multi-style not available
        pass
    
    # CURSOR CONSISTENT STYLE (Original formula)
    hl = headline.lower()
...
It's a BIG CLUB - you AIN'T in it! Never were! Never will be!

BOTH sides rob you blind!
...
Cops killing unarmed people with PHONES! They got guns, badges, QUALIFIED IMMUNITY - scared of US?!

"Defund" crowd - criminals gonna police THEMSELVES?! That's STUPID!

"Back the Blue" - until at YOUR door with a warrant!

EVERYBODY wrong! Fix the damn system!
...
Kids can't READ but can TIKTOK! Can't do MATH but can do FORTNITE!

Money for BOMBS not BOOKS! $800 BILLION for war! $3 for schools!

Republicans: "Poor kids stay dumb!" Democrats: "$100K unemployment degrees!"

Teachers need second jobs! CEOs need second YACHTS!
...
DRAFT?! Didn't learn from VIETNAM?! Rich men send POOR kids to die for OIL!

Republicans: "Support troops!" - by killing them overseas!

Democrats: "$800 BILLION war budgets!" while kids starve!

BOTH love war! Death is PROFITABLE!
...
Market crashes! Rich sad! Poor been CRASHED for YEARS!

Republicans: "Free market!" - unless YOU fail! Then bailouts!

Democrats: "Tax rich!" while BEING rich!

Poor pay TAXES! Rich pay ACCOUNTANTS! You work TWO jobs! Can't afford rent! That's modern SLAVERY!
...
Republicans: "Climate hoax!" - while buying FLOOD insurance!

Democrats: "Save Earth!" - from PRIVATE JETS! Lecturing YOU about carbon!

Planet's FINE! PEOPLE are fucked! Oil companies KNEW for 50 YEARS! LIED the whole time!

ALL complicit in your death!
...
Only KIDS and DRUNKS tell the truth! Everyone else LYING to your FACE!

Republicans: Small government in a UTERUS!

Democrats: Free everything - who PAYS?! SPOILER: YOU!
...
def generate_psychological_audio(duration, output_path):
    ...
    try:
        ...
        left_channel = mono_layer + binaural_left
        right_channel = mono_layer + binaural_right
...
def generate_bitcoin_qr():
    ...
    try:
        # Use Cash App QR code (600x600, high-contrast, scannable)
        cashapp_qr_path = BASE_DIR / "qr_codes" / "cashapp_qr.png"
        
        # Check if 600x600 Cash App QR exists
        ...
        print("[QR] Generating 600x600 Cash App QR code...")
        qr = qrcode.QRCode(
            version=1,
            error_correction=qrcode.constants.ERROR_CORRECT_M,
            box_size=20,  # Large boxes for scannability
            border=4,
        )
        qr.add_data(CASHAPP_LINK)
        qr.make(fit=True)
        
        # Create high-contrast image (white on black for dark videos)
        qr_img = qr.make_image(fill_color="white", back_color="black")
        
        # Resize to 600x600 (scannable from 2+ feet)
        ...
        # Save
        cashapp_qr_path.parent.mkdir(parents=True, exist_ok=True)
        canvas.save(str(cashapp_qr_path))
        
        print(f"[QR] Cash App QR code generated: {cashapp_qr_path.name} (650x650px)")
        return cashapp_qr_path
        ...
        # Fallback to Bitcoin QR if Cash App fails
...
def create_max_headroom_video(lincoln_image, audio_path, output_path, headline, use_lipsync=True, use_jumpscare=True):
    ...
    if qr_path and qr_path.exists():
        ...
        # Add QR code overlay to filter chain - NEURO-OPTIMIZED + HIGH VISIBILITY
        # Size: 400x400 (INCREASED for better visibility)
        # Position: Top-right w-420 (Z-pattern endpoint, 23% higher scan rate)
        # Science: Scannable 6-24 inches, maximum attention without intrusion
        # FIXED: Ensure QR is on TOP of all other elements
        vf_simple = vf_simple.replace('[vtext1]copy[vf]', f'[2:v]scale=400:400[qr];[vtext1][qr]overlay=w-420:20[vf]')
        map_video = '[vf]'
    else:
        # No QR code
        map_video = '[vf]'
    ...
    timeout_seconds = 300  # Increased to 5 minutes for complex VHS filter chain
...
#AbrahamLincoln #MaxHeadroom #Horror #Shorts #Viral #Glitch"""
        ...
        # OPTIMIZED TAGS (20-25 for maximum discoverability)
        # Import tag optimizer
        try:
            from YOUTUBE_TAG_OPTIMIZATION import generate_optimized_tags
            tags = generate_optimized_tags(headline, title, 'short')
        except:
            # Fallback to expanded tag list
            tags = [
                'abraham lincoln', 'lincoln horror', 'max headroom', 'max headroom style',
                'political satire', 'dark comedy', 'glitch aesthetic', 'vhs horror',
                'scary', 'creepy', 'horror', 'exposed', 'truth', 'warning',
                'shorts', 'youtube shorts', 'viral shorts', 'trending',
                'conspiracy', 'wake up', 'mind blowing', 'disturbing',
                'political horror', 'satire news', 'viral video'
            ]
