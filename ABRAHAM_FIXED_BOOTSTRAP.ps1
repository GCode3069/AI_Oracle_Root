# ═══════════════════════════════════════════════════════════════════════════
# ABRAHAM HORROR - FIXED BOOTSTRAP (NO ERRORS)
# ═══════════════════════════════════════════════════════════════════════════
...
Write-Host ""
Write-Host "🔥 ABRAHAM - FIXED GENERATOR" -ForegroundColor Red
...
# Create Python file
Write-Host ""
Write-Host "🐍 Creating Python generator..." -ForegroundColor Yellow
...
$pythonFile = Join-Path $ROOT "abraham_fixed.py"
$pythonContent | Set-Content -Path $pythonFile -Encoding UTF8 -Force

# Verify file was created
if (Test-Path $pythonFile) {
    $fileSize = (Get-Item $pythonFile).Length
    Write-Host "  ✅ Created: abraham_fixed.py" -ForegroundColor Green
    Write-Host "  ✅ File size: $fileSize bytes" -ForegroundColor Green
} else {
    Write-Host "  ❌ FAILED to create Python file!" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Set-Location $ROOT
python abraham_fixed.py $Count

# List generated videos
Write-Host ""
Write-Host "📋 Generated Videos:" -ForegroundColor Cyan
$videos = Get-ChildItem -Path "$ROOT\uploaded" -Filter "*.mp4"
if ($videos.Count -gt 0) {
    foreach ($video in $videos) {
        $size = [math]::Round($video.Length / 1MB, 2)
        Write-Host "  ✅ $($video.Name) ($size MB)" -ForegroundColor Green
    }
    Write-Host ""
    Write-Host "Total: $($videos.Count) videos" -ForegroundColor Yellow
} else {
    Write-Host "  ⚠️  No videos found in uploaded folder" -ForegroundColor Red
}

Write-Host ""
Write-Host "✅ COMPLETE!" -ForegroundColor Green
Write-Host ""
Write-Host "Bitcoin: $BTC" -ForegroundColor Cyan
