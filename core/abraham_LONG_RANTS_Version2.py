"""
ABRAHAM LINCOLN - LONG DARK SATIRICAL RANTS
Run directly: python abraham_LONG_RANTS.py 50
Each script 2500+ characters with deep headline roasts
"""
...
def long_rant(headline):
    """Generate 2500+ character dark satirical rant about the headline"""
    hl = headline.lower()
    
    # Detect headline type for specific roasts
    ...
    intro = f"""Abraham Lincoln here. Yes, THAT Abraham Lincoln. The tall guy. The beard guy. The guy who got assassinated at a fucking theatre in 1865 while watching a mediocre play.
...
And in those nine hours between the shot and death, something happened. Time collapsed. I saw past. Present. Future. All of it at once. Including this. Including you. Including {headline}"""

    closing = """So here we are. Me, dead for over 150 years. You, alive right now. This headline between us.
...
def audio(s, o):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            subprocess.run(["ffmpeg", "-i", str(t), "-af", "aecho=0.8:0.88:1000:0.3,atempo=0.94,bass=g=3", "-y", str(o)], capture_output=True)
            t.unlink()
            return True
...
def video():
    ...
    try:
        r = requests.get("https://api.pexels.com/videos/search", 
                        ...
                        params={"query": "dark fog cemetery", "per_page": 1, "orientation": "portrait"}, 
                        timeout=30)
        d = r.json()
        if d.get("videos"):
            v = d["videos"][0]
...
def compose(v, a, o):
    ...
    try:
        p = subprocess.run(["ffprobe", "-v", "error", "-show_entries", "format=duration", 
                          "-of", "default=noprint_wrappers=1:nokey=1", str(a)], 
                          capture_output=True, text=True)
        d = float(p.stdout.strip())
        subprocess.run(["ffmpeg", "-i", str(v), "-i", str(a), "-map", "0:v:0", "-map", "1:a:0", 
                       "-vf", "eq=contrast=1.7:brightness=-0.6:saturation=0.4,scale=1080:1920", 
                       "-c:v", "libx264", "-crf", "19", "-c:a", "aac", "-b:a", "320k", 
                       "-t", str(d), "-shortest", "-y", str(o)], 
...
def gen():
    ...
    s = long_rant(h)
    print(f"Script: {len(s)} chars >>> LONG DARK RANT <<<")
    ...
    up = BASE / "uploaded" / f"ABE_LONG_{t}.mp4"
...
if __name__ == "__main__":
    ...
    print(f"\n🔥 GENERATING {count} LONG ABE LINCOLN RANTS 🔥\n")
